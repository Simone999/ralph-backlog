...
- Worker and verifier last-message files should use `mktemp` under `TMPDIR` and be deleted before `run_codex` returns; shell regressions can wrap fixture-local `mktemp` to assert both chosen path and cleanup, and removing fixture `sleep` is a good guard that no fixed inter-iteration pause came back.
